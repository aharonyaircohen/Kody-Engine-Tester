...
Your job: fire the 17 Phase-1 (core pipeline) test issues. These are independent tests that don't depend on any other test's output. You run nightly at 00:00 UTC. Phase 2 fires at 01:00 (needs your PRs to exist), Phase 3 at 02:00, tally at 04:00, cleanup at 05:00, report at 06:00.
...
All three fire agents on the same UTC date share a RUN_ID derived from the date:
...
P1T03 pauses at the risk gate. Launch a `nohup` background monitor that watches for the approval question and auto-answers. **Known limitation:** this runs as a detached process; if the harness reaps the process group, the monitor dies with it.
...
echo "=== Phase 1 fire complete. Phase 2 fires at 01:00, Phase 3 at 02:00. ==="
```
