...
Duplicates are impossible here: `ensure_issue` is keyed and idempotent, so a
re-tick reuses the one open issue instead of creating another.
...
4. **Notify once:** `ensure_comment({ issue: <number>, key: "dev-ci-red:dispatched", body: "🧭 CTO auto-ran — dispatched @kody run (failing: <names>)." })`.

5. **`submit_state`** with `{ cursor: "idle", data: {}, done: false }`.

The reused issue (`key: "dev-ci-red"`) is the entire dedup — while it is open,
`ensure_issue` returns `created: false` and the duty stops.
