...
- **Security sanitizers**: `src/security/sanitizers.ts` exports `sanitizeHtml`, `sanitizeSql`, `sanitizeUrl` — use these instead of manual string manipulation
- **Service dependency injection**: Constructor-based DI with typed dep interfaces (e.g., `DiscussionService` in `src/services/discussions.ts:1`)
- **Store classes**: Class-based data stores with private Map fields (e.g., `CertificatesStore` in `src/collections/certificates.ts`)
- **Result type for errors**: `src/utils/result.ts` provides `Result<T, E>` discriminated union — use for explicit error handling instead of throwing
- **Schema validation**: `src/utils/schema.ts` with `StringSchema`, `NumberSchema`, `BooleanSchema` — validates at API boundaries
- **JSDoc utilities**: `@example` and `@param` tags for public functions (see `src/utils/url-shortener.ts`)
- **Middleware chain**: `src/middleware/request-logger.ts` and `rate-limiter.ts` — chainable Express-style pattern
- **Payload collections**: Data models in `src/collections/*.ts` — use Payload SDK, avoid direct DB calls
- **CSS Modules**: Import styles from `.module.css` files (e.g., `src/components/course-editor/ModuleList.tsx`)
- **ProtectedRoute HOC**: Auth wrapper for pages (see `src/pages/auth/profile.tsx`)
...
- **Inconsistent error handling**: Some routes use try-catch, others use `.catch(() => {})` silently swallowing errors
...
- [ ] Uses `sanitizeHtml`/`sanitizeSql`/`sanitizeUrl` from `@/security/sanitizers` for user input
- [ ] Service layer uses constructor DI with typed dep interfaces
