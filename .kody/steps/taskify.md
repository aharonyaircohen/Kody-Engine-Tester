...
- **Schema validation**: `src/middleware/validation.ts` exports `validate()` and `ValidateResult` discriminated union for request boundary validation
- **Auth HOC**: `src/auth/withAuth.ts` wraps handlers with JWT + RBAC via `checkRole(user, requiredRole)`
- **Result type**: `src/utils/result.ts` provides `Result<T, E>` discriminated union for explicit error handling
- **Service DI**: `src/services/` classes (e.g., `GradebookService`) receive typed deps via constructor (`GradebookServiceDeps`)
- **In-memory stores**: `src/collections/certificates.ts` uses private `Map` fields with public accessor methods
- **Security sanitizers**: `src/security/sanitizers.ts` — `sanitizeHtml`, `sanitizeUrl` (rejects `javascript:`/`data:`), `sanitizeSql`

## Improvement Areas

- **Dual auth systems**: `src/auth/user-store.ts` (SHA-256) coexists with `src/auth/auth-service.ts` (PBKDF2) — inconsistent password hashing
- **Role mismatch**: `UserStore.UserRole` uses `'admin'|'user'|'guest'|'student'|'instructor'` vs `RbacRole` uses `'admin'|'editor'|'viewer'`
- **Type casts in dashboard**: `src/app/(frontend)/dashboard/page.tsx` uses `as unknown as` instead of proper type guards
- **Validation middleware gap**: `src/middleware/validation.ts` exists but not integrated into all API routes
- **Seed data in collections**: `src/collections/contacts.ts` embeds `SEED_CONTACTS` array — should be in migrations
...
- [ ] Assumptions section surfaced for architectural/technology choices not derivable from code
