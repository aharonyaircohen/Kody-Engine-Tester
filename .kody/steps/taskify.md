...
- **Auth HOC**: `src/auth/withAuth.ts` wraps route handlers with JWT validation and RBAC via `checkRole` — use this for protecting API routes
- **Service DI**: `src/services/discussions.ts` uses constructor dependency injection with `private readonly` stores — follow this pattern for new services
- **Result Type**: `src/utils/result.ts` provides `Result<T, E>` discriminated union — use `Result.ok()` / `Result.err()` for explicit error handling
- **Payload Collections**: `src/collections/*.ts` define data models — use Payload SDK, avoid direct DB calls
- **Validation Middleware**: `src/middleware/validation.ts` uses `ValidationSchema` with type coercion — validate body/query/params at API boundaries
- **Middleware Chain**: `src/middleware/request-logger.ts` implements Express-style chainable pattern — use for request logging and rate limiting
- **CSS Modules**: Components use `.module.css` files co-located with components — import via `import styles from './Component.module.css'`

## Improvement Areas

- **Dual auth systems**: `src/auth/user-store.ts` (SHA-256) coexists with `src/auth/auth-service.ts` (PBKDF2) — prefer AuthService for consistency
- **Role mismatch**: `UserStore.UserRole` uses 5 roles vs `RbacRole` uses 3 — no alignment between systems
- **Type safety**: `src/app/(frontend)/dashboard/page.tsx` uses `as unknown as` casts — prefer proper type guards
- **N+1 risk**: Dashboard batches lesson fetches but other pages may miss optimization
...
- [ ] risk_level matches scope size and impact (low/medium/high heuristics)
...
- [ ] ASSUMPTIONS section reflects actual project tech stack (Next.js, Payload CMS, PostgreSQL, JWT)
