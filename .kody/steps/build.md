...
- **DI Container usage**: Register services with `container.register(token, factory)` and resolve via `container.resolve(token)`. See `src/utils/di-container.ts:54` for singleton lifecycle pattern.
- **Auth HOC wrapping**: Always wrap API routes with `withAuth` from `src/auth/withAuth.ts`; first arg is role requirement (e.g., `'admin'`, `'editor'`).
- **Service dependency injection**: Service constructors accept typed dep interfaces (e.g., `GradebookServiceDeps`), not concrete Payload instances.
- **Result type for errors**: Use `Result<T, E>` from `src/utils/result.ts` instead of throwing; pattern: `return ok(value)` or `err(error)`.
- **Store pattern**: In-memory stores use class with `private data = new Map()`; methods return `Promise<Result<...>>`.
- **Zod-style validation**: Use `src/utils/schema.ts` Schema classes at API boundaries — `StringSchema`, `NumberSchema`, `BooleanSchema` with `.optional()` and `.default()`.

## Improvement Areas

- **Dual auth systems**: `src/auth/UserStore.ts` uses SHA-256 while `src/auth/AuthService.ts` uses PBKDF2 — consolidate to one scheme in `AuthService`.
- **Role misalignment**: `UserStore.UserRole` ('admin'|'user'|'guest'|'student'|'instructor') doesn't match `RbacRole` ('admin'|'editor'|'viewer') — align before adding new roles.
- **Unsafe type casts**: `src/app/(frontend)/dashboard/page.tsx` uses `as unknown as` casts — replace with proper type guards using `Result` type or custom type predicates.
- **Missing error propagation**: Some `src/services/*.ts` catch errors but return `err()` without the original error attached — preserve error chain via `err(new Error(...))`.
- **N+1 queries**: Dashboard page batch-fetches lessons but `src/services/GradebookService.ts` may fetch enrollments individually — use Payload's `depth` param or batch queries.

## Acceptance Criteria

- [ ] All `src/services/*.ts` files return `Result<T, E>` (not raw throws) for explicit error handling
- [ ] `withAuth` HOC applied to every custom API route in `src/api/` and `src/routes/`
- [ ] `pnpm tsc --noEmit` passes with zero errors before PR is opened
- [ ] `pnpm test` (vitest + playwright) passes with no skipped/failing tests
- [ ] No `as unknown as` casts in `src/app/(frontend)/dashboard/page.tsx` or new code
- [ ] Role constants unified — single source of truth in `src/auth/roles.ts` or similar
- [ ] All new services accept dependencies via constructor injection (DI pattern)
- [ ] `vitest.setup.ts` global mocks/fixtures applied to all integration tests
- [ ] Payload migrations in `src/migrations/` are reversible and tested
- [ ] Playwright E2E tests use `seedTestUser()`/`cleanupTestUser()` fixture pattern
