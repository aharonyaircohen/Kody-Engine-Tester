...
- **DI Container**: Register factories in `src/utils/di-container.ts`; services use constructor injection with typed dep interfaces (e.g., `GradebookServiceDeps<T>`)
- **Auth HOC**: Wrap routes with `src/auth/withAuth.ts`; use `checkRole` for RBAC; extract token via `extractBearerToken`
- **Service Layer**: Constructor injection of store dependencies; async methods; see `src/services/DiscussionService.ts`
- **Repository/Store**: In-memory stores use private `Map`; co-locate with collection config in `src/collections/`
- **Middleware Chain**: `src/middleware/request-logger.ts` and `rate-limiter.ts` for logging/rate-limiting
- **Result Type**: Use `src/utils/result.ts` `Result<T, E>` for explicit error handling in services

## Improvement Areas

- **Dual auth systems**: `UserStore` (SHA-256) vs `AuthService` (PBKDF2) in `src/auth/` — inconsistent password hashing; consolidate on one approach
- **Role divergence**: `UserStore.UserRole` ('student'/'instructor') vs `RbacRole` ('editor'/'viewer') — no alignment; unify roles across stores
- **Type casts**: `dashboard/page.tsx` uses `as unknown as` casts; prefer proper type guards
- **N+1 risk**: Dashboard batch-fetches lessons; check `src/app/(frontend)/dashboard/page.tsx` for similar patterns

## Acceptance Criteria

- [ ] New code follows naming conventions (PascalCase for components/types, camelCase for functions)
- [ ] Service layer uses constructor injection with typed interfaces
- [ ] API routes wrapped with `withAuth` HOC where auth is required
- [ ] New files co-located in appropriate `src/` subdirectory (services/, collections/, utils/)
- [ ] Tests use `vi.fn()` + `mockResolvedValue` pattern for mocks
- [ ] No `as unknown as` casts — use proper type guards instead
- [ ] Run `pnpm lint` and `pnpm test` pass before marking done
- [ ] New Payload collections follow singular slug convention
- [ ] Security utilities use `sanitize*` naming and return empty string for invalid input
