...
- **DI Container**: `src/utils/di-container.ts` — use `Container.register()` with factory functions for new services
- **Auth HOC**: `src/auth/withAuth.ts` — wrap route handlers with `withAuth(handler, ['admin', 'editor'])`
- **Service pattern**: Constructor injection of store dependencies; e.g., `DiscussionService` at `src/services/discussions.ts:8-14`
- **Collection stores**: `Map<string, T>` pattern from `src/collections/certificates.ts` for in-memory state
- **Result type**: `src/utils/result.ts` — use `Result.ok()` / `Result.fail()` for explicit error handling

## Improvement Areas

- **Role divergence**: `UserStore.UserRole` vs `RbacRole` — no alignment (see `src/auth/withAuth.ts` vs `src/utils/di-container.ts`)
- **Type casting**: `dashboard/page.tsx` uses `as unknown as` casts — prefer proper type guards
- **N+1 risk**: Dashboard batch-fetches lessons; other pages may miss similar optimizations
- **Dual auth**: `UserStore` (SHA-256) coexists with `AuthService` (PBKDF2) — password hashing inconsistent

## Acceptance Criteria

- [ ] New services follow DI pattern (`Container.register<T>()`) from `src/utils/di-container.ts`
- [ ] Route handlers use `withAuth` HOC from `src/auth/withAuth.ts`
...
- [ ] Zod schemas in `src/validation/` for request DTOs
