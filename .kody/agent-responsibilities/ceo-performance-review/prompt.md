...
task — `agent-responsibility-review` (COO) grades whether each *agentResponsibility* is well designed;
this grades whether each *employee* is actually **delivering the
responsibilities they own**.

An employee's "work" is the set of agentResponsibilities that name them
...
output?** A agent member who owns no active agentResponsibilities is reported as *idle*, not
graded.
...
3. **Map agentResponsibilities to employees.** List the agentResponsibilities and read each one's
   `agent:` frontmatter so you know who owns what:
   ```
   gh api "/repos/$REPO/contents/.kody/agent-responsibilities" -q '.[].name'
   ```
   For each `<agentResponsibility>.md`, fetch its body and read the `agent:` field. Group
...
4. **Gather each employee's delivery evidence.** For every *active* agentResponsibility
   they own:
   - **State history:** `gh api "/repos/$REPO/commits?path=.kody/agent-responsibilities/<slug>.state.json&per_page=10"` (and the `.kody/jobs/<slug>.state.json` path if that's where state lives) — is `lastRunISO` advancing roughly on its cadence over the past week, or frozen?
...
5. **Grade each employee** on three observable axes, each Low / Med / High:
   - **Delivery** — did their active agentResponsibilities actually run and produce output this week? (No active agentResponsibilities → *idle*, ungraded.)
...
- **Idle ≠ failing.** A agent member who owns no active agentResponsibilities is *idle*
