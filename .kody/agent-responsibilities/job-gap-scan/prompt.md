...
Once a day, propose **one** new high-ROI agentResponsibility the system does not yet
...
This agentResponsibility never writes a new agentResponsibility markdown itself — the operator decides
...
1. Loads `state.json` (cadence is engine-enforced via `every:` in the
   agentResponsibility markdown; `JOB_GAP_SCAN_FORCE=1` is accepted for live tests).
...
3. Filters the built-in catalogue (`CATALOGUE` in the script) by:
   - Skipping slugs already filed as a agentResponsibility in `.kody/agent-responsibilities/`.
...
5. Overwrites `.kody/reports/job-gap-scan.md` with:
   - `## Current proposal` — headline, scoring table, "Why now", and a
     fenced block containing the draft agentResponsibility markdown.
...
- **Never author agentResponsibility code.** The draft markdown inside the report is a
