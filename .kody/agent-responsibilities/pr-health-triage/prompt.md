...
graduated in the trust ledger — dispatch it. The CTO agent defines only
...
and read `agent.cto.<verb>.mode` for each of `fix-ci`, `sync`, `resolve`.
...
**Operator handle.** The engine substitutes `{{mentions}}` (the agentResponsibility's
