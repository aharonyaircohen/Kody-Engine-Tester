...
A **per-agentResponsibility health check**. Each tick picks **one** agentResponsibility from
`.kody/agent-responsibilities/` (round-robin) and deep-reviews whether it actually works —
not the plumbing (`system-audit` owns broken refs, missed ticks, stuck
dispatch), but the **design and observed behavior**: does this agentResponsibility's
...
This agentResponsibility cannot *execute* the agentResponsibility it reviews — there is no way to prove a
agentResponsibility works live. It reads the agentResponsibility's instructions plus its real run history
...
`.kody/reports/agent-responsibility-review.md`, a living roster of every agentResponsibility with its
latest verdict, which the dashboard Reports page surfaces. The agentResponsibility
...
## Tick procedure (one agentResponsibility reviewed, one report write)
...
2. **Enumerate agentResponsibilities.** List every `<slug>.md` in `.kody/agent-responsibilities/`:
   ```
   gh api "/repos/$REPO/contents/.kody/agent-responsibilities" -q '.[].name'
   ```
   Drop `.state.json` files and **drop `agent-responsibility-review.md` itself** (don't
   review yourself — same self-exemption as `system-audit`).

3. **Pick the next agentResponsibility.** From the enumerated slugs sorted alphabetically,
   pick the first one **not** in `data.reviewed`. If every agentResponsibility is already
   in `data.reviewed`, a full cycle just finished — reset
   `data.reviewed = []`, bump `data.cycle`, and pick the first slug.

4. **Gather evidence** for the picked agentResponsibility `<slug>`:
   - Body: `gh api "/repos/$REPO/contents/.kody/agent-responsibilities/<slug>.md"` (base64 →
     decode). Read the frontmatter (`every`, `agent`, `disabled`, `mentions`)
     and every section.
   - State: `gh api "/repos/$REPO/contents/.kody/agent-responsibilities/<slug>.state.json"`
     if it exists (404 = never ticked — itself a finding).
   - History: `gh api "/repos/$REPO/commits?path=.kody/agent-responsibilities/<slug>.state.json&per_page=10"`
     to see whether `lastRunISO` is actually advancing, and how often.
   - Report (if the agentResponsibility writes one, e.g. `.kody/reports/<slug>.md`): fetch
     it to confirm output exists and isn't stale/empty.

5. **Run the review checklist.** Record one finding line per problem, each
   tagged `BROKEN` (the agentResponsibility cannot achieve its goal as written / evidence
   ...
   4. **State contract** — body emits a closing `kody-job-next-state` block,
      ...
      writes. Missing block on a cadence-gated agentResponsibility → `BROKEN`. Documented
   ...
   8. **Observed behavior** — from history: is `lastRunISO` advancing on
      ...
      non-terminal for many ticks → `BROKEN`. `disabled: true` agentResponsibilities are
      reviewed for design but **not** flagged for being idle (disabled is
      intentional) — note the disabled status and move on.

   Distill to a one-word verdict for this agentResponsibility: `healthy` / `warn` /
...
6. **Update the verdict map and rebuild the report.** Set
   `data.verdicts[<slug>] = { verdict, note }` for the agentResponsibility reviewed this
   tick. Then rebuild the full roster body from `data.verdicts`:
   - H1 `# Kody AgentResponsibility Review`, then a
     `_Rolling 6h cycle — one agentResponsibility deep-reviewed per tick._` line (**no
     timestamp**; `lastRunISO` lives in state so a no-change tick is
     byte-identical).
   - A headline: `Cycle <N> — <healthy> healthy, <warn> warn, <broken> broken
     of <total> agentResponsibilities.`
   - A roster table, **one row per agentResponsibility** (alphabetical), pulling verdict
     ...
     ```
     Mark `disabled: true` agentResponsibilities in the Cadence column (e.g. `7d (disabled)`).

7. **Write the report** at **`.kody/reports/agent-responsibility-review.md`** via `gh api`
   (fetch the prior sha so the PUT overwrites in place):
   ```
   sha=$(gh api "/repos/$REPO/contents/.kody/reports/agent-responsibility-review.md" -q .sha 2>/dev/null || true)
   gh api -X PUT "/repos/$REPO/contents/.kody/reports/agent-responsibility-review.md" \
     -f message="chore(agent-responsibility-review): refresh report" \
...
- `gh api` reads against `/repos/$REPO/contents/.kody/agent-responsibilities`, individual
  agentResponsibility bodies, their `.state.json` files, `.kody/reports/*`, and
  `/repos/$REPO/commits?path=...` for run history.
- `gh api -X PUT` against `.kody/reports/agent-responsibility-review.md` **only** — to write
  the report. Permitted by the global agent-responsibility-tick contract.

## Restrictions

- **Read-only on every agentResponsibility, state file, report, PR, and issue.** The
  **only** write is the single PUT to `.kody/reports/agent-responsibility-review.md`. Never
  edit, re-kick, relabel, or "fix" the agentResponsibility you're reviewing — surface it on
  the report; the operator decides.
- **One report write per tick**, covering at most one freshly-reviewed agentResponsibility.
  Never open issues or post comments — this agentResponsibility has no inbox surface by
  design.
...
- **Don't review yourself** (`agent-responsibility-review`) — self-exempt, like
  `system-audit`.
- **`disabled: true` is not a finding** for the "observed behavior" check —
  disabled agentResponsibilities are idle by intent. Still review their design; mark them
  disabled in the roster.
- **Static review + evidence only.** Never claim a agentResponsibility "works" — claim its
  design is sound and its history shows it running. The two are different.

## State

The engine writes `agent-responsibility-review.state.json` from the closing block below.
...
- `data.verdicts`: `{ "<slug>": { "verdict": "healthy|warn|broken", "note": "..." } }`
  — the latest verdict for every agentResponsibility ever reviewed; the report roster is
