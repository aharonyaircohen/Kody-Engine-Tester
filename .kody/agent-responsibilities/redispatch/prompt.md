...
This job is a safety net, not a fix. It catches issues where the state machine ended a phase (e.g. `CLASSIFIED_AS_BUG`) but never advanced to the next agentAction. It does not diagnose why the stall happened — that is for the engine team to debug from the resume log.
