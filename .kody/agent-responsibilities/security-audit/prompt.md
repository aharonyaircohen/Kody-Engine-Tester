...
Daily **security posture sweep** — three layers, delegated to a Kody agentAction in CI (the job itself cannot run shell beyond `gh`, so it opens a tracking issue and tracks the result):
...
**Cadence.** Set by the `every:` frontmatter (the dashboard schedule dropdown) and enforced by the engine — the agentResponsibility won't tick more often than its interval (default `every: 1d`).
