...
- **Profile-load validation**: each top-level duty action (`run`, `fix`, `fix-ci`, `resolve`, `review`, `plan`, `classify`, `spec`, `research`, `sync`, `init`, `duty-scheduler`, `duty-action-smoke`, `duty-tick`, `bug`, `feature`, `chore`, `ui-review`, `release`) is invoked and must either accept valid input or fail with a clear missing-flag error. A profile-load crash (e.g. `Cannot find module`) always fails.
