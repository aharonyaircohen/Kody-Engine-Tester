...
AgentActions are Kody's versioned action definitions: JSON profiles (`.kody/agent-actions/*/profile.json`) paired with natural-language prompts (`.kody/agent-actions/*/prompt.md`). The engine reads them at runtime and drives a Claude Code agent accordingly.
...
.kody/agent-actions/<name>/
  ...
  cases.json    — (optional) for test-style agentActions; static test vectors
  <other inputs>
```
...
| Field | Purpose |
|---|---|
| `name` | Unique agentAction identifier |
| `role` | e.g. `watch` (cron-triggered), `interactive` |
| `kind` | e.g. `scheduled` — distinguishes agentAction kinds |
...
## Invoking an agentAction

- **In-repo:** `kody run --agentAction <name> [--arg value ...]`
- **Published engine:** `npx -y -p @kody-ade/kody-engine@latest kody <name> [args]`

The `nightly-tests` agentAction uses the latter pattern to exercise the published package.
