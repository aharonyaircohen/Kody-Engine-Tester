...
- **Reason**: `PAYLOAD_SECRET` environment variable is not set, causing server-side error on homepage
- **Evidence**: Component IS correctly bundled - `src_components_welcome-banner_tsx_0f.c49h._.js` present in page scripts
- **Page error**: `Error: missing secret key. A secret key is needed to secure Payload.`

## Pre-existing Issues (NOT related to welcome banner)
All typecheck/lint/test failures below are pre-existing issues in the codebase, not caused by the welcome banner implementation.
