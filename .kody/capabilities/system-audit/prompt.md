...
Audit the coordination of capabilities and agent in `.kody/`. Walk the
...
dropdown) and enforced by the engine — the capability won't tick more often
than its interval (default `every: 30m`).

**Per tick (one action max):**

1. **Enumerate definitions** via the GitHub contents API:
   - Capabilities: `gh api "/repos/<owner>/<repo>/contents/.kody/capabilities" -q '.[].name'`
   - Agent: `gh api "/repos/<owner>/<repo>/contents/.kody/agents" -q '.[].name'`

   For each `<slug>.md` in `.kody/capabilities/`, fetch its body to read the
   frontmatter (`disabled`, `agent`, `every`). For each `<slug>.md` in
   `.kody/agents/`, the slug alone is enough. Also note which capabilities
...
2. **Run the seven checks.** For each violation, record one line in the
   ...
   1. **Broken agent reference** *(mechanical)* — capability's `agent:`
      field names a slug that does not exist in `.kody/agents/`.
      Fix: create `.kody/agents/<slug>.md` (identity-only) or correct
      the capability's `agent:` field.
   2. **Orphan agent** — agent file exists but no enabled capability
      references it. `cto`, `coo`, and the auditor's own agent are
      exempt if no capability currently uses them. No mechanical fix
      (operator decides whether to delete or leave as standby).
   3. **Missed tick** — capability is enabled, has an `every:` cadence, and
      its `state.json` `data.lastRunISO` is older than `now - 2 ×
      cadence`. Capabilities with `every: manual` or no cadence are skipped.
      No mechanical fix (often transient cron skew).
   4. **Missing state** *(mechanical)* — capability has been ticked (body
      ...
      the capability body so the engine writes `<slug>.state.json` on every
      tick.
   5. **Cooldown violated** — `data.lastRunISO` is more recent than
      ...
      List up to the 3 most recent violations per capability. No mechanical
      fix (often indicates an engine or capability-body bug worth
      investigating).
   ...
   7. **Duplicate dispatch** — same capability's `state.json` shows two
      `lastRunISO` commits within 60 seconds of each other in the
      last 24 hours of history. List capability slug and the timestamp pair.
...
3. **Pin the repo, then find or open the tracking issue.**
   ...
   gh issue create --repo "$REPO" \
     ...
     --body "Tracking issue for the system-audit capability. Each tick that finds violations posts a comment here so the operator sees it in the inbox. Read-only — never close."
   ```
...
- `gh api` reads against `/repos/<owner>/<repo>/contents/.kody/capabilities`,
...
- **Read-only on everything except the tracking issue.** Never edit,
  delete, rename, label, or re-kick any capability, agent member, state file,
  comment, PR, or other issue.
- **At most one issue comment per tick.** Only on the Kody system
  audit issue.
- **No file writes.** This capability never modifies the working tree.
- **Quiet on clean** — no comment when zero violations. The inbox
  is precious; do not spam it.
- Auditor's own capability (`system-audit`) and agent (`coo`) are exempt
...
- `data.lastRunISO`: UTC ISO timestamp of the last tick that ran past
  the cadence guard.
- `data.nextEligibleISO`: UTC ISO timestamp this capability will next be
