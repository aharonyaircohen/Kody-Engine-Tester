...
features that the changelog-verification capability (which only tests *new* entries)
never revisits.

This capability is **live** (no `disabled:` key → enabled by default). The repo is
...
and enforced by the engine — the capability won't tick more often than its interval.
...
5. **Otherwise** (none open) → open a tracking issue and dispatch with no
   ...
   how this capability used to stall for hours):
   ```
...
The `<!-- kody-capability: {{capability}} -->` line is **always required** (even on a clean
sweep, where the `kody-cmd:` line is omitted) — it keys trust to this capability.
Copy it verbatim.
