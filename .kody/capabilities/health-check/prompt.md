...
**Cadence.** Set by the `every:` frontmatter (the dashboard schedule dropdown) and enforced by the engine — the capability won't tick more often than its interval (default `every: 1d`).
...
- `gh api -X PUT` against `.kody/reports/health-check.md` only — to write the report. Permitted by the global capability-tick contract.
...
- `data.lastRunISO`: ISO timestamp of the last tick that wrote the report (or null)
- `data.lastStuckCount`: count of stuck tasks in the last report (or 0)
