...
3. **Try to dispatch the fix:** `dispatch_workflow({ executable: "run", issueNumber: <number> })`.
   The tool only fires when this capability is **trusted (Auto)**; if it's in **Ask**
...
4. **Notify once, per outcome:**
   ...
   - **Not dispatched (Ask)** → `ensure_comment({ issue: <number>, key: "dev-ci-red:awaiting", body: "🔴 dev CI is failing (<names>). Awaiting operator — grant this capability **Auto** on the dashboard Trust page to auto-dispatch the fix." })`. Do not dispatch.
