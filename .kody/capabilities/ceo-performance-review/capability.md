...
A **weekly review of every agent member**, the way a company reviews its
employees. The unit is the **person** (`.kody/agents/<slug>.md`), not the
task — `capability-review` (COO) grades whether each *capability* is well designed;
this grades whether each *employee* is actually **delivering the
responsibilities they own**.

An employee's "work" is the set of capabilities that name them
(`agent: <slug>`). Their delivery quality is read from the **evidence those
capabilities leave behind**: state files advancing on cadence, reports/comments
...
output?** A agent member who owns no active capabilities is reported as *idle*, not
graded.
...
3. **Map capabilities to employees.** List the capabilities and read each one's
   `agent:` frontmatter so you know who owns what:
   ```
   gh api "/repos/$REPO/contents/.kody/capabilities" -q '.[].name'
   ```
   For each `<capability>.md`, fetch its body and read the `agent:` field. Group
...
4. **Gather each employee's delivery evidence.** For every *active* capability
   they own:
   - **State history:** `gh api "/repos/$REPO/commits?path=.kody/capabilities/<slug>.state.json&per_page=10"` (and the `.kody/jobs/<slug>.state.json` path if that's where state lives) — is `lastRunISO` advancing roughly on its cadence over the past week, or frozen?
   - **Output:** any tracking issue the capability posts to, or `.kody/reports/<slug>.md` — did it produce real findings this week, or is it stale/empty? Repeated byte-identical no-op comments count as **churn**, not delivery.

5. **Grade each employee** on three observable axes, each Low / Med / High:
   - **Delivery** — did their active capabilities actually run and produce output this week? (No active capabilities → *idle*, ungraded.)
...
- **Idle ≠ failing.** A agent member who owns no active capabilities is *idle*
