...
```bash
STATE_FILE=".kody/agent-loops/test-suite-state.json"
```
...
push_state() {
  ...
  local sha
  sha=$(gh api repos/aharonyaircohen/Kody-Engine-Tester/contents/.kody/agent-loops/test-suite-state.json \
    --jq '.sha' 2>/dev/null || echo "")
  if [ -n "$sha" ]; then
    gh api repos/aharonyaircohen/Kody-Engine-Tester/contents/.kody/agent-loops/test-suite-state.json \
      --method PUT \
...
do_fire() {
  ...
  INITIAL_SHA=$(gh api repos/aharonyaircohen/Kody-Engine-Tester/contents/.kody/agent-loops/test-suite-state.json \
    --jq '.sha' 2>/dev/null || echo "")
