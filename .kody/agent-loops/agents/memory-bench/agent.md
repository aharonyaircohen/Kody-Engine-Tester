...
Check `.kody/agent-runs/*.jsonl` for issues with 2+ runs. Count how many trigger `!REPEAT_FAIL` or `!LOOP` warnings.
...
3. Check `.kody/agent-runs/{issue}.jsonl` for 2+ entries per issue
...
for f in .kody/agent-runs/*.jsonl; do
  lines=$(wc -l < "$f")
  if [ "$lines" -gt 1 ]; then
...
1. Comment `@kody rerun` on MF1 — wait for failure
2. Comment `@kody rerun` on MF2 — wait for failure
3. Check `.kody/agent-runs/{issue}.jsonl` for 2+ entries
4. Record whether contradiction warnings fire
