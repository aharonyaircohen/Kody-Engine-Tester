...
describe('AuthMiddleware', () => {
  ...
  it('should return 401 for blacklisted token', async () => {
    ...
    // Blacklist the token (simulating a logout or refresh)
    jwtService.blacklist(accessToken)
    const result = await middleware({ authorization: `Bearer ${accessToken}`, ip: '127.0.0.1' })
    expect(result.status).toBe(401)
  })

  it('should return 401 for token with older generation after blacklist', async () => {
    const user = await userStore.findByEmail('user@example.com')
    ...
    // Simulate refresh by blacklisting the old token
    jwtService.blacklist(oldAccessToken)
