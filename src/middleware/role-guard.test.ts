...
const makeContext = (role: string) => ({
  user: { id: '1', email: 'test@example.com', role, isActive: true } as unknown as AuthenticatedUser,
})
...
describe('requireRole', () => {
  ...
  it('should return 401 when user exists but has no role', () => {
    const guard = requireRole('admin')
    const result = guard({ user: { id: '1', email: 'test@test.com', role: '' } as unknown as AuthenticatedUser })
    expect(result?.status).toBe(401)
