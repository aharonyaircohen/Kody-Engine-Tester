...
describe('paginate', () => {
  ...
  it('should return all items when pageSize is larger than array length', () => {
    const items = [1, 2, 3]
  ...
  it('should throw an error for page <= 0', () => {
  ...
  it('should correctly calculate hasNext for single page', () => {
    const items = [1, 2]
    const result = paginate(items, 1, 2)
    expect(result.hasNext).toBe(false)
  })

  it('should correctly calculate hasNext when on second to last page', () => {
    const items = [1, 2, 3, 4, 5]
    const result = paginate(items, 2, 2)
    expect(result.hasNext).toBe(true)
  })
