...
describe('UserStore', () => {
  ...
  describe('seed users', () => {
    ...
    it('should have editor user seeded', async () => {
      const user = await store.findByEmail('editor@example.com')
      expect(user).toBeDefined()
      expect(user?.role).toBe('editor')
    })

    it('should have viewer user seeded', async () => {
  ...
  describe('tokenVersion', () => {
    it('should start at 0 for new users', async () => {
      const user = await store.findByEmail('admin@example.com')
      expect(user?.tokenVersion).toBe(0)
    })
    ...
    it('should increment tokenVersion', async () => {
      const user = await store.findByEmail('admin@example.com')
      const newVersion = await store.incrementTokenVersion(user!.id)
      expect(newVersion).toBe(1)
      const updated = await store.findById(user!.id)
      expect(updated?.tokenVersion).toBe(1)
    })

    it('should return -1 for unknown user', async () => {
      const newVersion = await store.incrementTokenVersion('unknown')
      expect(newVersion).toBe(-1)
    })
  })
})